...
| `translating-a-content-block.mp4` | 98.4s | A `<Translate>` region as one content block: the demo's two regions in code, then the Manager in Italian, a language added for the take with nothing translated, the Home block with the original beside the untranslated Italian and its two segments, the block's AI Translate filling both with the editor repainting on its own, the second segment typed over by hand in the formal register (`Sfogli il catalogo nella sua lingua.`) and saved, and the Tour block whose `<DontTranslate>` sentence is struck through and stays in English while AI Translate fills in the Italian around it. Filmed against a Manager build carrying translation-manager MR !2 (the block editor's Save keeping segment edits, and its repaint after AI Translate); until that ships, the live app drops a segment edit and needs a reload to show the AI result. Italian was removed after the take |
