...
| `managing-translations.mp4` | 152.6s | The Manager on its own, for the person who owns the copy, with no code on screen: the Translations page and its To-do queue, one machine draft opened, the cart line's `carrito` corrected to `cesta` (the word Spain uses) with Update Memory switched on, Save, the badge changing from the model's name to a person's, Auto-next and Never Translate explained, then the React demo switched to Spanish with the correction live and nothing deployed, and Manage Access with a Translator invited to Spanish only |
...
`getting-started` and `managing-translations` are tutorials, not synthetic cuts: each is one continuous recording
