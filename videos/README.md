...
| `ai-translator-settings.mp4` | 137.2s | The AI Translator tab, one switch at a time, each shown by what it does to a phrase that arrives from the app: auto-translate off and the phrase waits in the to-do list with no draft, on and the next phrase is drafted by the model in seconds, auto-detect on and a phrase that arrives already in Spanish is held as never-translate by itself; then Use AI Translations named as the on-demand side, and the added-language policy pointed to its own video |
