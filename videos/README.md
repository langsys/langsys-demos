...
| `usage-and-credits.mp4` | 113.5s | The two meters, on the pages that show them: Billing's overview, the Subscription page with this month's API usage units against the plan's allowance, the Credits page at the Store's scope with its balance, usage summary and the transactions each machine translation draws from the organization, then the project's own Billing tab with the monthly credit limit and auto recharge. Nothing is bought and no card form is opened. Re-shot 2026-09-09 with real balances on the Store scope (prepaid and free credits, usage summary, one transaction per machine translation); page loads behind a dip, no app splash |
